...
export class PozitifcubeHttpService {
  ...
  constructor( private http: HttpClient , private router: Router , public toastr: ToastrService , private translate: TranslateService) { 
    ...
    window.onresize = () => { this.scrollBarHorizontal = (window.innerWidth < 1200); };
    
  }

  panel_path =  this.hostnm == 'crm.finalevim.com.tr' ? ('/') : '/';
  base_path =  this.hostnm == 'crm.finalevim.com.tr' ? ('//'+this.hostnm+'/phpapi/') : 'https://finalevimphpapi.pozitifcube.com/';
  //base_path =  this.hostnm == 'ebusem.com' ? ('//'+this.hostnm+'/phpapi/') : 'https://shtoptanapi.pozitifcube.com/';

  // base_path =  this.hostnm == 'ebusem.com' ? ('//'+this.hostnm+'/phpapi/') : 'https://ebusem.com/phpapi/';

  httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json', 'X-App-Path': this.router.url }) }

  loggedInPath = this.base_path+'querybuild';

  loggenInHttpOptionsArray(){
      // sadece content-type yok, buradaki amaç "file reader" vs içerisinde header göndermek
      return [
        {name: 'X-App-Path', value: this.router.url }, {name: 'X-Auth-Token',  value: localStorage.getItem('usertokeny') },
        {name: 'X-Auth-User', value: localStorage.getItem('usertokenx') }, {name: 'X-App-Lang', value: localStorage.getItem('userlang') }
      ];
  ...
  loginPost(user){
    return this.http
      .post(this.base_path+'connect', JSON.stringify(user) , this.httpOptions )
      .pipe(
        retry(2),
        catchError(this.handleError)
      )
  ...
  logoutPost(query){
    return this.http.post(this.base_path+'connect' , query , this.loggedInHttpOptions() )
    .pipe(
      retry(2),
      catchError(this.handleError),
      map(res => { return this.dynAuthControl(res); })
